...
M0–M3 are complete. M4 Minimal Messaging is active in final repository-wide verification #54 after completion of implementation slices #50–#53. The OCI host does not expand the current product or protocol milestone scope and must not be used as a reason to introduce M5 calling/WebRTC behavior. After M4 exact-main closure, #59/M4.5 is the next permitted milestone and M5 remains blocked until that closed messaging-alpha gate is complete.
