...
14. creates or resumes a draft GitHub Release targeted at the exact verified `main` commit, attaches the verified APK/AAB/checksum, publishes the release (which creates the protected `vX.Y.Z` tag when no tag exists yet), then verifies the published tag resolves to that exact commit;
