...
Kenato's backend is a deliberately small Go service. M2 provides authenticated identity publication and invite/contact establishment. M3 adds authenticated asynchronous session bootstrap. M4 is active: the protocol/auth contract is complete and the current #51 slice adds bounded durable storage for opaque encrypted message envelopes; authenticated WSS routing is still #52.
...
- no public identity, session-bootstrap, or mailbox lookup/search endpoint;
...
The server binds to `127.0.0.1:8080` by default. `KENATO_LISTEN_ADDR` may override the listener only for a reviewed deployment boundary. Public deployment is expected to terminate TLS in front of the loopback service; M4 #51 does not itself add a public WSS listener.
...
The built-in M2/M3 rate gate is intentionally process-wide: at most four concurrent gated operations and at most 600 accepted gated operations per one-minute process window. Source-aware/distributed edge limits should be implemented only together with the reviewed reverse-proxy/TLS deployment boundary rather than by trusting arbitrary forwarded-address headers in the loopback service. WSS-specific connection/backpressure limits remain #52.
...
The #51 mailbox core is not a public endpoint. It is consumed through an internal service contract that will be connected to authenticated WSS sessions in #52.
...
The mailbox enforces count/byte limits both in Go and with a SQLite `BEFORE INSERT` trigger. Physical global quotas include expired rows awaiting cleanup so delayed maintenance cannot make disk use unbounded.
...
Repository verification additionally runs `scripts/verify_m4_protocol.py` and `scripts/verify_m4_mailbox.py`. CI builds the server for linux/amd64 and linux/arm64 and validates all current protocol schemas with `protoc`.
