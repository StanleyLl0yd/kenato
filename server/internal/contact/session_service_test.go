...
func (s *identityMemoryStore) PublishIdentity(context.Context, PublicationRecord) (uint64, error) {
...
func (s *identityMemoryStore) CreateInvite(context.Context, [sha256.Size]byte, []byte, int64, int64) (int64, error) {
	return 0, errors.New("unused")
}

func (s *identityMemoryStore) RedeemInvite(context.Context, RedeemRecord) (RedeemResult, error) {
	return RedeemResult{}, errors.New("unused")
}

func (s *identityMemoryStore) ClaimInvite(context.Context, ClaimRecord) (ClaimResult, error) {
	return ClaimResult{}, errors.New("unused")
}
...
func TestSessionServicePublishRequiresKenatoIdentityBinding(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("publish valid bootstrap: %v", err)
	}
	if generation != 1 || revision != 1 {
		t.Fatalf("unexpected accepted generation/revision %d/%d", generation, revision)
	}

	bundle.OlmCurve25519IdentityKey[0] ^= 0xff
	if _, _, err := service.PublishSessionBootstrap(context.Background(), PublishSessionBootstrapRequest{
		ProtocolVersion: SessionProtocolVersion,
		Bundle:          bundle,
	}); err == nil {
...
func TestSessionServiceReserveRejectsIdentitySubstitution(t *testing.T) {
	...
	sessionStore.reserved = StoredSessionReservation{
		CreatorBundle:     encoded,
		CreatorOneTimeKey: creatorSession.OneTimePreKeys[0],
	}
	service := newSessionServiceWithClock(identityStore, sessionStore, func() int64 { return 1000 })
	...
	if _, err := service.ReserveSessionBootstrap(context.Background(), request); err != nil {
		t.Fatalf("reserve valid bootstrap: %v", err)
	}
...
func TestSessionServiceSubmitBindsRedeemerGenerationAndCiphertext(t *testing.T) {
	...
	if err := service.SubmitSessionInit(context.Background(), request); err != nil {
		t.Fatalf("submit valid init: %v", err)
	}
	if sessionStore.init == nil || !bytes.Equal(sessionStore.init.OlmMessage, request.OlmMessage) {
		t.Fatal("session init was not stored")
	}

	request.RedeemerAccountGeneration++
	payload, _ = SessionSubmitPayload(request)
	request.SubmitSignature = signPayload(t, redeemerKey, payload)
	if err := service.SubmitSessionInit(context.Background(), request); !errors.Is(err, ErrSessionConflict) {
...
func newIdentityMemoryStore(t *testing.T, bundles ...PublicIdentityBundle) *identityMemoryStore {
	...
	for _, bundle := range bundles {
		...
		if err != nil {
			t.Fatalf("encode identity bundle: %v", err)
		}
...
func newSignedSessionBundle(t *testing.T, identityID []byte, identityKey *ecdsa.PrivateKey, generation, revision uint64, oneTimeCount int) SessionBootstrapBundle {
	...
	for i := 0; i < oneTimeCount; i++ {
		...
		bundle.OneTimePreKeys = append(bundle.OneTimePreKeys, SessionOneTimePreKey{
			ID:        uint64(i + 1),
			PublicKey: key,
		})
	}
	...
	if err != nil {
		t.Fatalf("session bootstrap payload: %v", err)
	}
...
func TestSessionCanonicalBindingSignatureUsesSHA256ECDSA(t *testing.T) {
	...
	if !ecdsa.VerifyASN1(&key.PublicKey, digest[:], bundle.BindingSignature) {
		t.Fatal("binding signature does not verify")
	}

	bad := bytes.Clone(bundle.BindingSignature)
	if len(bad) == 0 {
		t.Fatal("empty test signature")
	}
	bad[len(bad)-1] ^= 1
	if ecdsa.VerifyASN1(&key.PublicKey, digest[:], bad) {
		t.Fatal("tampered signature verifies")
	}
}
...
func TestSessionBootstrapRejectsNonCanonicalKeyOrdering(t *testing.T) {
	...
	if _, err := SessionBootstrapPayload(bundle); err == nil {
		t.Fatal("duplicate session one-time key accepted")
	}
}
...
func TestSessionSubmitProofRejectsCiphertextTamper(t *testing.T) {
	...
	payload, _ := SessionSubmitPayload(request)
	request.SubmitSignature = signPayload(t, redeemerKey, payload)
	if err := VerifySessionSubmitProof(request, redeemer.IdentityPublicKey); err != nil {
