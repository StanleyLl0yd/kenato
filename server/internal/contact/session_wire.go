...
func DecodeReserveSessionBootstrapRequest(data []byte) (ReserveSessionBootstrapRequest, error) {
	...
	err := consumeFields(data, func(number protowire.Number, wireType protowire.Type, value []byte) (int, error) {
		...
		case 3:
			...
			raw, n, err := consumeBoundedBytes(wireType, value, IdentityIDBytes)
			if err == nil {
				request.RedeemerIdentityID = bytes.Clone(raw)
			}
			return n, err
		case 4:
			if seen&8 != 0 {
		...
		case 5:
			...
			if err == nil {
				request.ReserveSignature = bytes.Clone(raw)
			}
			return n, err
		default:
			return consumeUnknown(number, wireType, value)
...
func EncodeReserveSessionBootstrapResponse(bundle SessionBootstrapBundle, key SessionOneTimePreKey) ([]byte, error) {
	...
	if len(out) > MaxSessionWireMessageBytes {
...
func DecodeSubmitSessionInitRequest(data []byte) (SubmitSessionInitRequest, error) {
	...
	err := consumeFields(data, func(number protowire.Number, wireType protowire.Type, value []byte) (int, error) {
		...
		case 4:
			...
			return n, err
		case 5:
			if err := bit(16); err != nil {
				return 0, err
			}
			parsed, n, err := consumeUint64(wireType, value)
			request.CreatorAccountGeneration = parsed
			return n, err
		case 6:
			if err := bit(32); err != nil {
				return 0, err
			}
			parsed, n, err := consumeUint64(wireType, value)
			request.CreatorOneTimePreKeyID = parsed
			return n, err
		case 7:
			if err := bit(64); err != nil {
				return 0, err
			}
			parsed, n, err := consumeUint64(wireType, value)
			request.RedeemerAccountGeneration = parsed
			return n, err
		case 8:
			if err := bit(128); err != nil {
				return 0, err
			}
			parsed, n, err := consumeUint32(wireType, value)
			request.OlmMessageType = parsed
			return n, err
		case 9:
			if err := bit(256); err != nil {
				return 0, err
			}
			raw, n, err := consumeBoundedBytes(wireType, value, MaxSessionCiphertextBytes)
			if err == nil {
				request.OlmMessage = bytes.Clone(raw)
			}
			return n, err
		case 10:
			if err := bit(512); err != nil {
				return 0, err
			}
			raw, n, err := consumeBoundedBytes(wireType, value, MaxSignatureBytes)
			if err == nil {
				request.SubmitSignature = bytes.Clone(raw)
			}
			return n, err
		default:
			return consumeUnknown(number, wireType, value)
...
func EncodeClaimSessionInitResponse(result ClaimSessionInitResult) ([]byte, error) {
	...
	if !validSignature(result.RedemptionSignature) || result.RedeemedAt < 0 ||
		result.CreatorAccountGeneration == 0 || result.CreatorAccountGeneration > math.MaxInt64 ||
		result.OlmMessageType != OlmMessageTypePreKey || len(result.OlmMessage) == 0 ||
		len(result.OlmMessage) > MaxSessionCiphertextBytes || !validSignature(result.SubmitSignature) {
	...
	out = appendUint64Field(out, 7, result.CreatorAccountGeneration)
	out = appendUint32Field(out, 8, result.OlmMessageType)
	out = appendBytesField(out, 9, result.OlmMessage)
	out = appendBytesField(out, 10, result.SubmitSignature)
	if len(out) > MaxSessionWireMessageBytes {
		return nil, ErrMalformedWire
	}
	return out, nil
...
func EncodeSessionBootstrapBundle(bundle SessionBootstrapBundle) ([]byte, error) {
	...
	if len(out) == 0 || len(out) > MaxWireMessageBytes {
		return nil, ErrMalformedWire
	}
	return out, nil
}
...
func DecodeSessionBootstrapBundle(data []byte) (SessionBootstrapBundle, error) {
	if len(data) == 0 || len(data) > MaxWireMessageBytes {
		return SessionBootstrapBundle{}, ErrMalformedWire
	}
	...
	err := consumeFields(data, func(number protowire.Number, wireType protowire.Type, value []byte) (int, error) {
		...
		case 2:
			...
			seen |= 2
			parsed, n, err := consumeUint64(wireType, value)
			bundle.AccountGeneration = parsed
			return n, err
		case 3:
			if seen&4 != 0 {
				return 0, ErrMalformedWire
		...
		case 4:
			...
			seen |= 8
			raw, n, err := consumeBoundedBytes(wireType, value, OlmPublicKeyBytes)
			if err == nil {
				bundle.OlmEd25519IdentityKey = bytes.Clone(raw)
			}
			return n, err
		case 5:
			if seen&16 != 0 {
			...
			raw, n, err := consumeBoundedBytes(wireType, value, OlmPublicKeyBytes)
			if err == nil {
				bundle.OlmCurve25519IdentityKey = bytes.Clone(raw)
			}
			return n, err
		case 6:
			if len(bundle.OneTimePreKeys) >= MaxSessionOneTimePreKeys {
				return 0, ErrMalformedWire
			}
		...
		case 7:
			...
			raw, n, err := consumeBoundedBytes(wireType, value, MaxSignatureBytes)
			if err == nil {
				bundle.BindingSignature = bytes.Clone(raw)
			}
			return n, err
		default:
			return consumeUnknown(number, wireType, value)
	...
	if err != nil {
		return SessionBootstrapBundle{}, err
	}
	if err := validateSessionBootstrapShape(bundle, true); err != nil {
		return SessionBootstrapBundle{}, err
	}
...
func encodeSessionOneTimePreKey(key SessionOneTimePreKey) ([]byte, error) {
	if key.ID == 0 || key.ID > math.MaxInt64 || len(key.PublicKey) != OlmPublicKeyBytes || allZero(key.PublicKey) {
		return nil, ErrInvalidSessionBootstrap
	}
	var out []byte
...
func decodeSessionOneTimePreKey(data []byte) (SessionOneTimePreKey, error) {
	if len(data) == 0 || len(data) > 128 {
		return SessionOneTimePreKey{}, ErrMalformedWire
	}
	...
	if err != nil || key.ID == 0 || key.ID > math.MaxInt64 || len(key.PublicKey) != OlmPublicKeyBytes || allZero(key.PublicKey) {
		return SessionOneTimePreKey{}, ErrMalformedWire
	}
	return key, nil
}
...
func validateSessionWireSize(data []byte) error {
	if len(data) == 0 || len(data) > MaxSessionWireMessageBytes {
		return ErrMalformedWire
	}
	return nil
}
